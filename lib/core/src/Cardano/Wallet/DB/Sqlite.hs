...
withDBLayer
    ...
    -> ((SqliteContext, DBLayer IO s k) -> IO a)
       -- ^ Action to run.
    -> IO a
withDBLayer trace defaultFieldValues mDatabaseDir addrScheme =
    bracket before after
  where
    before = newDBLayer trace defaultFieldValues mDatabaseDir addrScheme
...
newDBFactory
    ...
    -> Maybe FilePath
       -- ^ Path to database directory, or Nothing for in-memory database
    -> IO (DBFactory IO s k)
newDBFactory tr defaultFieldValues addrScheme = \case
    Nothing -> do
        -- NOTE
        ...
        pure DBFactory
            { withDatabase = \wid action -> do
                db <- modifyMVar mvar $ \m -> case Map.lookup wid m of
                    ...
                    Nothing -> do
                        (ctx, db) <-
                            newDBLayer tr defaultFieldValues Nothing addrScheme
...
newDBLayer trace defaultFieldValues mDatabaseFile addrScheme = do
...
class PersistState s k where
    ...
    insertState :: (W.WalletId, W.SlotId) -> s -> AddressScheme k-> SqlPersistT IO ()
    -- | Load the state for a checkpoint.
    selectState :: (W.WalletId, W.SlotId) -> AddressScheme k -> SqlPersistT IO (Maybe s)
...
instance PersistState (Rnd.RndState) k where
    ...
    selectState (wid, sl) _ = runMaybeT $ do
